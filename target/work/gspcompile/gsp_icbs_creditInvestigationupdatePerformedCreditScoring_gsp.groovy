...
class gsp_icbs_creditInvestigationupdatePerformedCreditScoring_gsp extends GroovyPage {
public String getGroovyPageFileName() { "/WEB-INF/grails-app/views/creditInvestigation/updatePerformedCreditScoring.gsp" }
public Object run() {
Writer out = getOut()
...
printHtmlPart(24)
invokeTag('select','g',53,['noSelection':(['':'-Set Score Here-']),'id':("xcode"),'name':("xcode"),'from':(icbs.lov.CreditScoringAnswers.findAllByStatusAndCode(true,'charAnswer',[sort: "id", order: "asc"])),'optionKey':("id"),'optionValue':("description"),'value':(icbs.loans.CreditScoringChecklistRecords.findByCreditScoringQuestions(questionXs)?.creditScoringAnswers?.id),'class':("many-to-one form-control")],-1)
printHtmlPart(25)
...
printHtmlPart(24)
invokeTag('select','g',69,['noSelection':(['':'-Set Score Here-']),'id':("xcode"),'name':("xcode"),'from':(icbs.lov.CreditScoringAnswers.findAllByStatusAndCode(true,'charAnswer',[sort: "id", order: "asc"])),'optionKey':("id"),'optionValue':("description"),'value':(icbs.loans.CreditScoringChecklistRecords.findByCreditScoringQuestions(xquestionXs)?.creditScoringAnswers?.id),'class':("many-to-one form-control")],-1)
printHtmlPart(25)
...
invokeTag('form','g',80,['name':("myForm"),'id':("saveFormCreditScoringDetails"),'url':([action:'updatesaveCreditScoringChecklist',controller:'creditInvestigation']),'method':("POST")],3)
printHtmlPart(32)
})
