    ...
    def authenticate() {
        ...
        if(user) {
           ...
           // }
            ...
            // Check if user expiryDate will expire
            // by jm marquez
            if((user.expiryDate - branchRunDate) <= 7) {
                def noOfDaysLeft = user.expiryDate - branchRunDate
                def messageContentNotice = ""
                if(noOfDaysLeft == 0){
                    messageContentNotice = "Your User access will expire today, to avoid locking of account please Contact Administrator."
                }else{
                    messageContentNotice = "Your User access will expire in "+noOfDaysLeft+" day(s), to avoid locking of account please Contact Administrator."
                }
                def cr8MessageNoticeInstance = new UserMessage()
                cr8MessageNoticeInstance.subject = "User Access System Expiration Notice"
                cr8MessageNoticeInstance.sender = UserMaster.get(25)
                cr8MessageNoticeInstance.recipient = UserMaster.get(session.user_id)
                cr8MessageNoticeInstance.configItemStatus = ConfigItemStatus.get(2)
                cr8MessageNoticeInstance.isRead = false
                cr8MessageNoticeInstance.sentAt = new Date().format('yyyy-MM-dd HH:mm')
                cr8MessageNoticeInstance.body = messageContentNotice
                //println("Notification Message sent to username: "+usermessage.userMaster.name+"[ID: "+usermessage.userMaster.id+"]")
                cr8MessageNoticeInstance.save flush:true 
            }
        ...
        if (!policydetails) {
            ...
            if (!txnPolicy) {
                ...
                if(!isAllowedToOverride) {
                    ...
                    def txnLogWithOverrideDetails = new TxnLogWithOverrideDetails(userOverrideBy: UserMaster.findByUsername(params.username),
                        requestBy: UserMaster.get(session.user_id),description: 'authorized processing of '+policyDesc,
                        dateOverride: UserMaster.get(session.user_id).branch.runDate,txnTimestamp: new Date().toTimestamp(),txnFile: null)
                    txnLogWithOverrideDetails.save(flush:true)
                }            
            }
        } else {
