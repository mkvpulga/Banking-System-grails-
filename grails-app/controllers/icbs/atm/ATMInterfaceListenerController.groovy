...
class ATMInterfaceListenerController {
    ...
    def createTransaction(){ // TRANSACTION
        ...
        while(i < 128 && k < reqMsgLength){
           ...
           // println "i :"+i
            if(c == '1'){ // Element is active
                ...
                println("================= Element From ATMBitMap Table =================")
                //println("element: "+element)
                //println("element.isVar True or False ?: "+element.isVar)																						   
        ...
        println("****************************************")
        println("TERMINAL ID : "+elementArray[39])
        println("****************************************")
       

        if(!elementArray[39].equals("")){ // Terminal no.
            atmTxnInstance.atmTerminal = elementArray[39]
        }
        ...
        if(!elementArray[9].equals("")){ // Txn. date
            atmTxnInstance.txnRef = 'S'  + elementArray[9]
        }
        if(!elementArray[60].equals("")){ // Txn. date
            atmTxnInstance.txnRef = atmTxnInstance.txnRef + 'T' + elementArray[60]
        }        
            ...
            if(acct1){ // Account exists!
                ...
                if((acct1.status == DepositStatus.get(5)) || (acct1.status == DepositStatus.get(7)))
                    ...
                    if((new String(txnType) == "30") || (new String(txnType) == "92")){ // INQUIRY || MINI STATEMENT
                        println 'inquiry..'
                        if (!elementArray[26].equals("")){
                            
                            // inquiry with charge
                            def inqChg = elementArray[26].toDouble()*(1/100)
                            if (inqChg > acct1.availableBalAmt){
                                ...
                                def txnFileInstance2 = new TxnFile(acctNo:acct1.acctNo, depAcct:acct1, currency:acct1.product.currency, 
                                    branch:acct1.branch, txnAmt:inqChg, txnCode:ATMTxnTemplate2.code, txnDate:acct1.branch.runDate, 
                                    txnTimestamp:new Date().toTimestamp(), txnDescription:'BALINQ Charges', status:2, 
                                    txnParticulars:'BALINQ', txnType:ATMTxnTemplate2.txnType, txnTemplate:ATMTxnTemplate2, user:ATMUSerID,
                                ...
                                def acctLedgerInstance2 = new TxnDepositAcctLedger(acct:txnFileInstance2.depAcctId, acctNo:acct1.acctNo, 
                                    branch:branchInstance.id, debitAmt:inqChg, txnDate:branchInstance.runDate, 
                                    bal:acct1.ledgerBalAmt-inqChg,txnFile:txnFileInstance2.id, currency:acct1.product.currency, status:2, 
                                    txnType:ATMTxnTemplate2.txnType, txnRef:atmTxnReference2, txnCode:ATMTxnTemplate2.code,user:ATMUSerID)
                                    acctLedgerInstance2.save(flush:true) 
                                        
                                acct1.ledgerBalAmt -= inqChg          
                                acct1.availableBalAmt -= inqChg
                                acct1.save(flush:true)
                                                                            
                                atmTxnInstance.txnFile2 = txnFileInstance2.id
                        ...
                        if(!elementArray[2].equals("") || !elementArray[26].equals("")){ // Txn amt and charge amount
                            atmTxnInstance.txnAmt = elementArray[2].toDouble()*(1/100)
                            // check for charges
                            def chgAmt = 0.00D
                            if(!elementArray[26].equals("")){
                               chgAmt = elementArray[26].toDouble()*(1/100)
                            }
                                ...
                                if (!productTxnInstance){
                                    ...
                                    if (chgAmt > 0) {
                                        ...
                                        def acctLedgerInstance2 = new TxnDepositAcctLedger(acct:txnFileInstance2.depAcctId, acctNo:acct1.acctNo, 
                                            ...
                                            bal:acct1.ledgerBalAmt-chgAmt,txnFile:txnFileInstance2.id, currency:acct1.product.currency, status:2, 
                    ...
                    } else if ((new String(txnType) == "21"))  {  // IBFT
                        // check first if debit or credit will be performed
                        // for debit transaction check balance
                    } else {
                        if(!acctNo2.equals("")){
                          if(!(new String(txnType) == "10")){
                            ...
                            if(acct2){ // Account2 exists!
                                ...
                                if((acct2.status == DepositStatus.get(5)) || (acct2.status == DepositStatus.get(7)))
                                    ...
                                    if((new String(txnType) == "40")){ // Fund Transfer
                                        if(params.reqCode == "0200"){
