...
import icbs.audit.TxnLogWithOverrideDetails
@Transactional
...
//        }
        ...
        // this is for savings
        println("session Type Value: "+session["type"])
         if (session["type"] == 1 || session["type"] == 2) {
             ...
             if (reprintPb) {
                 ...
                 tf = Integer.valueOf(params.ledgerId.toString().replace(',',''))
                 unposted = TxnDepositAcctLedger.findAllByAcctAndIdGreaterThanEquals(acct,tf, [sort: "id", order: "asc"])                 
                 ledger = null
                 println 'reprint......'
                 println acct
                 println tf
            ...
            if (pbLineNo > 1){
             ...
             if (unposted) {
                 println '************************************************'
                 println("Build Unposted Transcation")
                 println unposted
                 for (up in unposted) {
                     ...
                     pbLine.append(up.txnFile.txnTemplate.amlaCode).append("|")
                     if (up.debitAmt > 0) {
                    ...
                    //for interest credit Quarter append to passbook
                /*   if(up.txnType.id == 31){
                        if (up.creditAmt > 0) {
                            def cdf = new DecimalFormat("###,###,##0.00");
                            pbLine.append(cdf.format(up.creditAmt)).append("|")
                     ...
                     pbLine.append(String.format("%02d",pbLineNo)).append("|")
                      pbLine.append("||")
                        ...
                        println '=======>'+up.txnRef
                        println up
                     println up.txnType.id
                     if(up.acct.type.id == 2 && (up.txnType.id == 6 || up.txnType.id == 7)){
                         println '%%%%%%'+up.txnRef
                ...
                // add current transaction   
                 if (ledger) {   
                     println("================= ADD CURRENT TRANSACTION ==============")
                     ...
                     if (pbLineNo == 1 && ledger.acct.dateOpened != ledger.acct.branch.runDate) {
                         ...
                         pbLine.append("|").append(ledger.acct.maturityDate.format("MM/dd/yyyy"))
                         def intstRate = new DecimalFormat("###,###,##0.00");
                         pbLine.append("|") 
                         pbLine.append(intstRate.format(ledger.acct.interestRate))
                     ...
                     curLine.append(ledger.txnFile.txnTemplate.amlaCode).append("|")
                     if (ledger.debitAmt > 0) {
                    ...
                    if (ledger.creditAmt > 0) {
                     ...
                     //for interest credit Quarter append to passbook
                      /*  if(ledger.txnType.id == 31){
                            if (ledger.creditAmt > 0) {
                                def cdf = new DecimalFormat("###,###,##0.00");
                                curLine.append(cdf.format(ledger.creditAmt)).append("|")
                            } else {
                                curLine.append("|").append("")
                            } 
                        }*/
                     curLine.append(String.format("%02d",pbLineNo)).append("|")
                      curLine.append("||")
                           curLine.append(acct.id)
                     println '=======>'+ledger.txnRef
                     println ledger.txnType.id
                     if(ledger.acct.type.id == 2 && (ledger.txnType.id == 6 || ledger.txnType.id == 7)){
                         println '%%%%%%'+ledger.txnRef
            ...
            if (pbLineNo > 1){
             ...
             if (unposted) {
                 for (up in unposted) {
                     ...
                     pbLine.append(String.format("%02d",pbLineNo))
                     pbLine.append("|")
                     pbLine.append(up.acct.maturityDate.format("MM/dd/yyyy")).append("|")
                     def instRtexx = new DecimalFormat("###,###,##0.00");
                     pbLine.append(instRtexx.format(up.acct.interestRate)).append("|")
                     pbLine.append(acct.id)
                     list.add(pbLine)
                 ...
                 if (ledger) {
                    ...
                    if (ledger.creditAmt > 0) {
                     ...
                     def cbdf = new DecimalFormat("###,###,##0.00");
                     curLine.append(cbdf.format(ledger.bal)).append("|")  
                     //add accrued Interest to passbook report
                     def xxDepositAcrintAmount = ledger.acct.acrintAmt? ledger.acct.acrintAmt.toDouble() : 0.00D  
                     println("xxDepositAcrintAmount: "+xxDepositAcrintAmount)
                     curLine.append(cbdf.format(xxDepositAcrintAmount)).append("|")
                     
                     curLine.append(String.format("%02d",pbLineNo))
                     curLine.append("|")
                     curLine.append(ledger.acct.maturityDate.format("MM/dd/yyyy")).append("|")
                     def instRtessstt = new DecimalFormat("###,###,##0.00");
                     curLine.append(instRtessstt.format(ledger.acct.interestRate)).append("|")
                      curLine.append(acct.id)
                }
                list.add(curLine)
             } else {
             ...
             if (pbLineNo > 9) {
                 ...
                 if (ledger) {
                     ...
                     curLine.append("|")
                     curLine.append(ledger.acct.maturityDate.format("MM/dd/yyyy")).append("|")
                     def instRte = new DecimalFormat("###,###,##0.00");
                     curLine.append(instRte.format(ledger.acct.interestRate)).append("|")
                     curLine.append(acct.id)
                 }                 
                 list.add(curLine)
                 println '----' + list
    ...
    def addCheckAjax() {      
        ...
        def checkStatus = CheckStatus.get(3)
//        add d2
...
//                            }
    ...
    def changeLoanDetails(){
        ...
        if(!loanInstance){
            ...
            def totalDue = 0.00D
            if(!totalintpd){
                // it means na walang laman un total due principal
                totalintpd = 0.00D
            }
            if(!totalint){
                // it means na walang laman un total due interest
                totalint = 0.00D
            }
...
//                println "Loan taxBalanceAmount: " + loanInstance?.taxBalanceAmount
                render(template: '/tellering/details/loanDetails', model: [totalPDue:totalPDue, loanInstance:loanInstance,id_loan:id_loan, prevTxnDepAcctLedgerId:prevTxnDepAcctLedgerId, pbvalidate:pbvalidate]) as JSON
            return
        }
    }
    ...
    def search(Integer max) {
        println "START DATEEEEEEEEEEEEEEEE: "+params.startdate
        println "END DATEEEEEEEEEEEEEEEEE :"+params.enddate
        println params
...
/* Validation Slip Printing */    
    ...
    def newTransactionReversalPrintValidation(){
         println params.txnFile
         // def xxtransactionFileId = session.transactionFileId
         def xxtransactionFileId = params.txnFile.toInteger()
         def xxmap = session.map
         try {  
            ...
            params._format ="PDF"
            params._file ="Validation_Reversal.jasper" //jasper file
            params.txnID = xxtransactionFileId //parameter name & value
    ...
    def printTransactionSlip(){    
        ...
        try {    
            ...
            {   
                ...
                params._format ="PDF"//required caps
                params._file ="OR.jasper" //jasper file
...
/* CASH FROM VAULT */
    ...
    def saveTellerCashFromVaultTxn(TxnTellerCash tc, TxnFile tf){       
        ...
        if(tc.hasErrors() || tf.hasErrors()){
            ...
            def txnLogWithOverrideDetailsInstance = TxnLogWithOverrideDetails.findByRequestByAndTxnfile(UserMaster.get(session.user_id),null)
            if(txnLogWithOverrideDetailsInstance){
                txnLogWithOverrideDetailsInstance.txnfile = tf
                txnLogWithOverrideDetailsInstance.save(flush:true,failOnError:true)
            }
...
// other check receipt general
    ...
    def saveTellerOtherCheckReceiptAdjustmentTxn(TxnFile tc){
        ...
        if(tc.hasErrors()){
            ...
            def txnLogWithOverrideDetailsInstance = TxnLogWithOverrideDetails.findByRequestByAndTxnfile(UserMaster.get(session.user_id),null)
            if(txnLogWithOverrideDetailsInstance){
                txnLogWithOverrideDetailsInstance.txnfile = tc
                txnLogWithOverrideDetailsInstance.save(flush:true,failOnError:true)
            }
    ...
    def saveTellerCashDepositTxn(TxnDepositAcctLedger tc, TxnFile tf){
        ...
        if(tc.hasErrors() || tf.hasErrors()){
            ...
            def temp = ((params.creditAmt).replace(',','')).toDouble();
             depacctname = depositInstance.acctName
            session["defacct_name"] = depacctname
            println("deposit account name: "+depacctname)         
            def txnTemp = tf.txnTemplate
...
/* CASH WITHDRAWAL */
    ...
    def saveTellerCashWithdrawalTxn(TxnDepositAcctLedger tc, TxnFile tf){
        ...
        if(tc.hasErrors() || tf.hasErrors()){
            ...
            depacctname = depositInstance.acctName
            session["defacct_name"] = depacctname
            println("withdraw from deposit account name: "+session["defacct_name"])												  
...
//            For passbook validation
           ...
           // tc.save(flush:true,failOnError:true)
            ...
            def txnLogWithOverrideDetailsInstance = TxnLogWithOverrideDetails.findByRequestByAndTxnfile(UserMaster.get(session.user_id),null)
            def userRoleInstance = UserRole.findByUserMaster(UserMaster.get(session.user_id))
            if(userRoleInstance.role.id == 2){
                if (tf.txnAmt>=100000){
                    if(txnLogWithOverrideDetailsInstance){
                    txnLogWithOverrideDetailsInstance.txnfile = tf
                    txnLogWithOverrideDetailsInstance.save(flush:true,failOnError:true)
...
//                println "TEST SEQ.2 " + txnSeqNo
            ...
            tc.otherAmt = (params?.otherAmt.replaceAll(",","")).toDouble()
...
/* CHECK ENCASHMENT */
    ...
    def saveTellerCheckEncashmentTxn(TxnCOCI tc, TxnFile tf){
       ...
       if(tc.hasErrors() || tf.hasErrors()){
            ...
            //tc.status = ConfigItemStatus.get(2)
            tc.cleared = 'TRUE'
            tc.status=ConfigItemStatus.read(3)
            tc.checkStatus = CheckStatus.read(5)			
            tc.save(flush: true)
...
//            println "TESTING " + loanInstance.product + "-" + txnTemp
            ...
            else
            {
             ...
             Double totalOthersPaid = 0
            if (params?.otherAmt)
            {
                totalOthersPaid = Double.parseDouble(params?.otherAmt.replaceAll(",",""))
            }
            else
            {
                totalOthersPaid = 0
            ...
            if(totalInterestPaid > loanInstance.interestBalanceAmount && totalPrincipalPaid < loanInstance.overduePrincipalBalance){
                flash.message = 'ERROR: Interest or Penaly payment should not be more than the amount due'
                render(view:'/tellering/txnLoanCashSpecifiedRepayment/create', model: [txnLoanCashSpecifiedRepaymentInstance:tc])       
                return                
            }
//            if(totalPenaltyPaid > loanInstance.penaltyBalanceAmount && loanInstance.overduePrincipalBalance > 0){
//                flash.message = 'ERROR: Interest or Penaly payment should not be more than the amount due'
//                render(view:'/tellering/txnLoanCashSpecifiedRepayment/create', model: [txnLoanCashSpecifiedRepaymentInstance:tc])       
//                return 
//            }
...
//                println "TEST SEQ.2 " + txnSeqNo
            ...
            def loanLedgerEntry = new LoanLedger(loan: loanInstance, txnFile: tf, txnDate: branch.runDate, txnTemplate: tf.txnTemplate, txnRef: tf.txnRef, //txnType: TxnType.get(11),
                principalCredit: totalPrincipalPaid, principalBalance: loanInstance.balanceAmount, 
                interestCredit: totalInterestPaid, interestBalance: loanInstance.interestBalanceAmount, 
                penaltyCredit: totalPenaltyPaid, othersCredit: totalOthersPaid, penaltyBalance: loanInstance.penaltyBalanceAmount,
                chargesCredit: totalServiceChargePaid, chargesBalance: loanInstance.serviceChargeBalanceAmount)
            loanLedgerEntry.save(flush:true,failOnError:true)
...
//            if(totalInterestPaid > loanInstance.interestBalanceAmount && loanInstance.overduePrincipalBalance < 0){
//                flash.message = 'ERROR: Interest payment should not be more than the amount due'
//                render(view:'/tellering/txnLoanCheckSpecifiedRepayment/create', model: [txnLoanCheckSpecifiedRepaymentInstance:tc])       
//               return              
//            }
//            if(totalPenaltyPaid > loanInstance.penaltyBalanceAmount && loanInstance.overduePrincipalBalance < 0){
//                flash.message = 'ERROR: Penaly payment should not be more than the amount due'
//                render(view:'/tellering/txnLoanCheckSpecifiedRepayment/create', model: [txnLoanCheckSpecifiedRepaymentInstance:tc])       
//               return              
//           }
//            
...
/* TRANSACTION REVERSAL/CANCELLATION */
        ...
        if (params) {
            ...
            } else
            {
                ...
                if ((tf.txnType.id == 3) || (tf.txnType.id == 4) || (tf.txnType.id == 9) || (tf.txnType.id == 27)){
                    if (tf.txnAmt > tf.depAcct.ledgerBalAmt) {
                        flash.message = 'Cannot reverse - negative ledger balance result |error|alert'
                        redirect(controller: "tellering", action: "viewTellerReverseCancelTxn")   
                        return                        
                    }
                }
                ...
                // Teller Cash Transfer
                if (tf.txnType.id == 2) {
                    println("Reverse Teller Cash Transfer")
                    println("Transaction File ID: "+tf.id.toInteger())
                    
                    def checkPolicyExceptionCheck = PolicyException.findByRecordId(tf.id)
                    if(checkPolicyExceptionCheck){
                        println("checkPolicyExceptionCheck: "+checkPolicyExceptionCheck)
                        println("checkPolicyExceptionCheck.requestingUser: "+checkPolicyExceptionCheck.requestingUser)
                        println("tf.branch.runDate: "+UserMaster.get(session.user_id).branch.runDate)
                        checkPolicyExceptionCheck.dateOfAction = UserMaster.get(session.user_id).branch.runDate
                        checkPolicyExceptionCheck.approvingUser = UserMaster.get(checkPolicyExceptionCheck.requestingUser.id)
                        checkPolicyExceptionCheck.save(flush: true)
                    }
                ...
                if (tf.txnType.id == 4) {
                   dep.ledgerBalAmt -= tf.txnAmt
                   dep.unclearedCheckBalAmt -= tf.txnAmt
                }
                ...
                if ((tf.txnType.id == 12) || (tf.txnType.id == 13) || (tf.txnType.id == 14) || (tf.txnType.id == 15)) {
                   ...
                   def ll = new LoanLedger(loan:loan, txnTemplate:tf.txnTemplate, txnDate:tf.txnDate, txnRef:'TXN REV-'+tf.txnRef,
                        ...
                        penaltyDebit:lp.penaltyAmt, othersDebit:lp.otherAmt, penaltyBalance:loan.penaltyBalanceAmount, chargesDebit:lp.serviceChargeAmt,
   ...
   // }
    ...
    def txnSuccess() {
        ...
        // new condition for breakdown printing for 
        // txnType id 1,2 and 23 beside the validation slip
        
        
    }
    def processIDforSSSCollectionTxnSuccess(){
        def json = request.JSON
        def sql = new Sql(dataSource)
        //println("json.txnfileID.: "+json.txnfileID)
        println("params: "+params)
        
        session["transactionFileId"] = params.txnFileId.toInteger();
        transactionFileId =  params.txnFileId.toInteger();
        //def txnFileInstance = TxnFile.get(params.txnFileId)
        //def txnReceivedSuccessInstance = TxnFile.get(session["transactionFileId"])
        redirect(controller: "tellering", action: "txnSuccess")        
    }
...
def createDepositCOCITxn(){
        println("=========== CREATE DEPOSIT COCI TXN =========")
        def user = UserMaster.get(session.user_id)
        println("user: "+ user)
        println("xxxx")
        if (user.cash){
           // println("cccc")+ user.cash
            println("user.cash : ")
            def txnPaymentAdjustmentInstance = new TxnFile()
            [txnPaymentAdjustmentInstance:txnPaymentAdjustmentInstance]
        }
        else{
            println user.cash
            flash.message = 'Error! No cash account defined.|error'
    ...
    def saveDepositCOCItxn(TxnFile tc){
        println("TXNFILE: "+tc)
        if(tc.hasErrors()){
            flash.message = 'Failed to transfer.|error'
            return
