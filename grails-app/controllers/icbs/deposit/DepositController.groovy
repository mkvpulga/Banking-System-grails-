...
//           def PassbookList = db.rows("select (select passbook_no from passbook where issue_passbook_id = A.id) from issue_passbook A where passbooks_idx = (select max(passbooks_idx) from issue_passbook) and deposit_id = ?",[depositInstance.id])
            def PassbookList = db.rows("select (select passbook_no from passbook where issue_passbook_id = A.id) from issue_passbook A  where passbooks_idx = (select max(passbooks_idx) from issue_passbook where deposit_id = ?) and deposit_id = ?",[depositInstance.id , depositInstance.id])
          def ListPassNo
           ...
           println depositInstance.passBookNo
           respond depositInstance
        }
    }
        ...
        if(params.checkClearing){
            ...
            if (f.empty) {
                ...
                if(bankformat == 2){
                    ...
                    buildAub(cmd)
                    def c = InwardClearingLoad.list()
                    for(a in c){
                        println("a: "+a.chequeNo)
                        updateClearingLoadAcctnameAndAcctNo(a.chequeNo)
                    ...
                    try {
                        for (String line = reader.readLine(); line != null; line = reader.readLine()) {
                            ...
                            if(fields.length==5){
                                println("condition fields.length==5 passed....")
                                println fields
                                ...
                                saveToTempInwardLoad.acctNo = cmd2.acctNo
                                saveToTempInwardLoad.chequeNo =cmd2.chequeNo
                                saveToTempInwardLoad.amt = cmd2.amt
                                saveToTempInwardLoad.brstn = cmd2.brstn
                                saveToTempInwardLoad.trancd = cmd2.trancd
    ...
    def updateClearingLoadAcctnameAndAcctNo(Long chequeNo){

        def xChequeNo = chequeNo.toLong()
        def sql = new Sql(dataSource)
        def testssss = "select B.acct_no, B.acct_name, A.cheque_no from inward_clearing_load A left join cheque C on C.cheque_no = A.cheque_no left join chequebook D on D.id = C.chequebook_id left join deposit B on B.id = D.deposit_id where A.cheque_no = "+xChequeNo
        def zxcFFFF = sql.rows(testssss)
        /*def sqlstmt = "update inward_clearing_load set acct_name = (select B.acct_name from inward_clearing_load A left join cheque C on C.cheque_no = A.cheque_no left join chequebook D on D.id = C.chequebook_id left join deposit B on B.id = D.deposit_id where C.cheque_no = "+xChequeNo+"),ca_acct_no = (select B.acct_no from inward_clearing_load A left join cheque C on C.cheque_no = A.cheque_no left join chequebook D on D.id = C.chequebook_id left join deposit B on B.id = D.deposit_id where C.cheque_no = "+xChequeNo+") where cheque_no ="+xChequeNo  
        def eexecutingUpdateSql = sql.executeUpdate(sqlstmt)*/
        for(iccupdt in  zxcFFFF){
            def updateColumnAcctname = InwardClearingLoad.findByChequeNo(iccupdt.cheque_no)
            updateColumnAcctname.acctName = iccupdt.acct_name
            updateColumnAcctname.caAcctNo = iccupdt.acct_no

            updateColumnAcctname.save(flush: true)
            
        }
        //println("eexecutingUpdateSql: "+eexecutingUpdateSql)
    }
    ...
    def buildAub(inwardCheckClearingCommand cmd){
        ...
        try {
            for (String line = reader.readLine(); line != null; line = reader.readLine()) {
                ...
                if(AUBrecordCode=="01"){
                    println('')
                    println("Record Code: "+AUBrecordCode)
                    println("Check Number: "+AUBcheckNumber)
                    println("BRSTN: "+AUBBRSTN)
                    println("Account Number: "+AUBaccountNumber)
                    println("Transaction Code: "+AUBtransactionCode)
                    println("Amount: "+AUBamount)
                    println('')

                    def cmd2 = new checkCommand()
                    cmd2.chequeNo = AUBcheckNumber.toLong()
                    cmd2.brstn = AUBBRSTN
                    cmd2.trancd = AUBtransactionCode
                    cmd2.acctNo = AUBaccountNumber
                    cmd2.amt =Double.parseDouble(AUBamount).div(100)
...
/* Memo Transaction Validation Printing */
	 ...
	 def depositTransactionValidationSlip(){
          println("params :"+params)
         def txnFileID = params.txnFile.toInteger()
         println("txnFileID : "+ txnFileID)
         try {
            println "Deposit Transaction"
            params._name = "Validation Slip Deposit Transaction"
            params._format = "PDF"
            params._file = "ValidationSlip.jasper"
            params.txnID =  txnFileID
...
class checkCommand{
    ...
    static constraints={
        ...
        chequeNo validator: { val, obj,errors ->
            ...
            println("check no val : "+val)
            def forRemarks = InwardClearingLoad.findByChequeNo(val)
            forRemarks.remarks = ""
            println("forRemarks: "+forRemarks)
            if(!cheque){
                errors.rejectValue('chequeNo','Cheque no. does not exist!')
                forRemarks.remarks = "Cheque no. does not exist!"
            }else{
                if(cheque.status.id!=2){
                     errors.rejectValue('chequeNo','Cheque no. is used or has not been activated yet!')
                     forRemarks.remarks = "Cheque no. is used or has not been activated yet!"
                } else {
                    ...
                    if (stopChq) {
                        ...
                        forRemarks.remarks = "Cheque Stopped"
                    }
                }
            }
            forRemarks.save(flush: true)
        }

        amt validator: { val, obj,errors ->
            def chequeInstance  = Cheque.findByChequeNo(obj.chequeNo)
            println("obj.chequeNo: "+obj.chequeNo)
            println("val: "+val)
            //========================================
            
            //========================================
            def forRemarks = InwardClearingLoad.findByChequeNo(obj.chequeNo)
            ...
            if(chequeInstance && chequeInstance.chequebook != null){
                ...
                if (depositInstance.status.id  == 5) {
                    ...
                    */
                    ...
                    forRemarks.remarks = forRemarks.remarks + " Insufficient Funds on Deposit Accounts and its sweep accounts"
                }else{
                    return true
                }
            }
