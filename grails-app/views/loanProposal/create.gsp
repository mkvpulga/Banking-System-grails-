...
<html>
	<head>
    		...
    		validateInputs = function(event) {
			    ...
			    var callback = function(data,params) {
			    	...
			    	var maxInterestRate = parseFloat($(data).find('#max-interest-rate').html());
                                ...
                                console.log("callback ufnction");
                                console.log("installment type: "+installmentType);
                                console.log("installmentCalculation: "+installmentCalculation);
        ...
        function pmt(interestRate, numPeriods, presentValue, futureValue) {
                ...
                var instamt;
                instamt = presentValue * interestRate * (Math.pow(1+interestRate,numPeriods)) / (Math.pow(1+interestRate,numPeriods)-1);
                return instamt;
        }                

	function updateInstallmentAmount() {
                                console.log("pasok sa updateInstallmentAmount");
				var amount = parseFloat($('#amount').val().replace(",",""));				
				var numInstallments = $('#numInstallments').val();
				var interestRate = $('#interestRate').val() / 100;
                                //var interestRate = ($('#interestRate').val() /100) / 12;
				...
				if (amount > 0 && numInstallments > 0) {
					var installmentAmount = 0;
                                      ...
                                      {
                                           console.log("compoundingRate: "+compoundingRate); 
					   installmentAmount = pmt(compoundingRate, numInstallments, amount);
	...
	<body>
        <content tag="breadcrumbs">
            <span class="fa fa-chevron-right"></span><span class="current">Create Loan Application Template</span>
	</content>
        ...
        <content tag="main-actions">
            <ul>                    
				...
				<li><g:link class="create" action="loanAmountCalculator">Calculate Loanable Amount (Monthly)</g:link></li>
			</ul>
        </content>
	</body>
</html>
