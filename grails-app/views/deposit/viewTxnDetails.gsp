...
<html>
    ...
    <body>
		...
		<script>
                function asd(){
                    var w = window.open("${g.createLink(controller: 'deposit', action: 'depositTransactionValidationSlip', params: [txnFile:txnFileInstance?.id])}",'_blank');
                    w.print();        
                }
            </script>
            <ul>
                <li><g:link action="depositInquiry" id="${txnFileInstance?.depAcct?.id}">Deposit Inquiry</g:link></li>
                <li><a onclick="javascript:asd()" params="${[txnId: txnFileInstance.id]}">Print Transaction Details</a></li>
            </ul>
        </content>        
    </body>    
</html>
