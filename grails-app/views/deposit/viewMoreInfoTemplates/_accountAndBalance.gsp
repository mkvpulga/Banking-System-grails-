        ...
        <table class="table table-bordered table-striped">
            <tbody>
                ...
                <tr>
                    ...
                    <g:if test="${depositInstance.passBookNo}">
                    <td width="75%"><g:formatNumber format="###############" id="NoPass" number="${depositInstance.passBookNo}"/></td>
                    </g:if>
                    <g:else>
                        <td width="75%" style="color:red">No Passbook Issued</td>
                        </g:else>
                </tr>
                <g:if test="${depositInstance?.type?.id==3}">
                <tr>
...
<div class="row" style="margin:auto">
    <div class="section-container">
        <fieldset>
            <legend class="section-header">Schemes Information</legend>
                <table class="table table-bordered table-striped">
                    <tbody>
                        <tr>
                            <td style="font-weight:bold" width="30%">Deposit Interest Scheme</td>
                            <td width="75%">${depositInstance?.depositInterestScheme?.name}"</td>
                        </tr>
                        <tr>
                            <td style="font-weight:bold" width="30%">Deposit Tax Charge Scheme</td>
                            <td width="75%">${depositInstance?.depositTaxChargeScheme?.name}"</td>
                        </tr>
                            <g:if test="${depositInstance?.type?.id == 3}">
                                <tr>
                                    <td style="font-weight:bold" width="30%">Fixed Deposit Pre Term Scheme</td>
                                    <td width="75%">${depositInstance?.fixedDepositPreTermScheme?.name}"</td>
                                </tr>
                                <tr>
                                    <td style="font-weight:bold" width="30%">Fixed Deposit Term Scheme</td>
                                    <td width="75%">${depositInstance?.fixedDepositTermScheme?.name}"</td>
                                </tr>
                            </g:if>    
                            <tr>
                                <td style="font-weight:bold" width="30%">GL Link</td>
                                <td width="75%">${depositInstance?.glLink?.description}</td>
                            </tr>
                            <tr>
                                <td style="font-weight:bold" width="30%">Product</td>
                                <td width="75%">${depositInstance?.product?.name}</td>
                            </tr>
                    </tbody>
                </table>    
        </fieldset>
