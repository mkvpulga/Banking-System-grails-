...
<%@ page import="icbs.deposit.InwardClearingLoad" %>
<html>
    <head>
    ...
    <body>
        <content tag="main-content">
            ...
            <div class="row">
                <g:form name="processInwardCheckingForm" action="processInwardCheckClearing"controller="deposit">
                        ...
                        <table id="clearingTable" class="table table-hover table-condensed table-bordered table-striped">
                            ...
                            <g:each var="check" in="${domainInstance?.checks}" status="i">
                                <tr>
                                    ...
                                    <td>${i + 1}</td>
                                    <td>${check?.acctNo}</td>
                                    <g:if test="${InwardClearingLoad.findByChequeNo(check?.chequeNo)}">
                                        <td>${InwardClearingLoad.findByChequeNo(check?.chequeNo).acctName}</td>
                                    </g:if>
                                    <g:else>
                                        <td></td>
                                    </g:else>
                                    <g:if test="${InwardClearingLoad.findByChequeNo(check?.chequeNo)}">
                                        <td>${InwardClearingLoad.findByChequeNo(check?.chequeNo).caAcctNo}</td>
                                    </g:if>
                                    <g:else>
                                        <td></td>
                                    </g:else> 
