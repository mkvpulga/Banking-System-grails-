...
<div class="fieldcontain form-group ${hasErrors(bean: depositInstance, field: 'branch', 'has-error')} ">
    ...
    <div class="col-sm-8">
        <g:select id="branch" name="branch.id" from="${icbs.admin.Branch.list(sort: "name", order: "asc")}" optionKey="id" optionValue="name"value="${branch?.id}" class="many-to-one form-control"/> 
...
<div class="fieldcontain form-group ${hasErrors(bean: depositInstance, field: 'reference', 'has-error')} ">
    ...
    <div class="col-sm-8">
        <g:textArea name="reference" cols="40" rows="5" maxlength="255" value="${holdInstance?.remarks}"class="form-control"/>
    </div>    
</div>
