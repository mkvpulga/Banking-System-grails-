...
<%@ page import="icbs.tellering.TxnSssCollection" %>
...
<html>
                        ...
                        <div class="infowrap">
                            <table class="table table-bordered table-striped">
                                <legend class="section-header">Transaction Details</legend>
                                    ...
                                    <g:if test="${txnFileInstance?.txnType?.id != 23 && txnFileInstance?.txnType?.id != 2 && txnFileInstance?.txnType?.id != 1}">
                                        <tr>
                                    ...
                                    <g:if test="${icbs.tellering.TxnSssCollection.findByTxnFile(txnFileInstance)}">   
                                        <tr>
                                            <td><label>Payment Code</label></td>
                                            <td>${icbs.tellering.TxnSssCollection.findByTxnFile(txnFileInstance).paymentCode}</td>
                                        </tr>
                                         <tr>
                                            <td><label>Payment Type</label></td>
                                            <td>${icbs.tellering.TxnSssCollection.findByTxnFile(txnFileInstance).paymentType}</td>
                                        </tr>
                                         <tr>
                                            <td><label>Payor Type</label></td>
                                            <td>${icbs.tellering.TxnSssCollection.findByTxnFile(txnFileInstance).payorType}</td>
                                        </tr>
                                         <tr>
                                            <td><label>SSS Number</label></td>
                                            <td>${icbs.tellering.TxnSssCollection.findByTxnFile(txnFileInstance).ssNumber}</td>
                                        </tr>
                                         <tr>
                                            <td><label>Starting Period</label></td>
                                            <td>${icbs.tellering.TxnSssCollection.findByTxnFile(txnFileInstance).startingPeriod}</td>
                                        </tr>
                                         <tr>
                                            <td><label>Ending Period</label></td>
                                            <td>${icbs.tellering.TxnSssCollection.findByTxnFile(txnFileInstance).endingPeriod}</td>
                                        </tr>
                                         <tr>
                                            <td><label>Ending Period</label></td>
                                            <td><g:formatNumber format="###,###,##0.00" number="${icbs.tellering.TxnSssCollection.findByTxnFile(txnFileInstance).ssAmountPaid}"/></td>
                                        </tr>
                                         <tr>
                                            <td><label>SS Amount Paid</label></td>
                                            <td><g:formatNumber format="###,###,##0.00" number="${icbs.tellering.TxnSssCollection.findByTxnFile(txnFileInstance).ssAmountPaid}"/></td>
                                        </tr>
                                         <tr>
                                            <td><label>EC Amount Paid</label></td>
                                            <td><g:formatNumber format="###,###,##0.00" number="${icbs.tellering.TxnSssCollection.findByTxnFile(txnFileInstance).ecAmountPaid}"/></td>
                                        </tr>
                                    </g:if>
                                    <tr>
    ...
    <script>
        ...
        function genReportTLR001(){
                        ...
                        reportlink = "${icbs.admin.Institution.findByParamCode("GEN.10251").paramValue}${icbs.admin.Report.get(20).baseParams}&output=${icbs.admin.Report.get(20).outputParam}";
        ...
        <%-- function genReportTSLP001(){
                        reportlink = "${icbs.admin.Institution.findByParamCode("GEN.10251").paramValue}${icbs.admin.Report.get(62).baseParams}&output=${icbs.admin.Report.get(62).outputParam}";
                        reportlink = reportlink + "&reportUnit=${icbs.admin.Report.get(62).reportUnit}";
                        reportlink = reportlink + "&txn_file_id=${txnFileInstance.id}";
                        reportlink = reportlink + "&generatedby=${icbs.admin.UserMaster.get(session.user_id).username}";
                        sendtojasperver(reportlink);
                    } --%>
            function genReportNewTransactionSlip(){
                        reportlink = "${icbs.admin.Institution.findByParamCode("GEN.10251").paramValue}${icbs.admin.Report.get(62).baseParams}&output=${icbs.admin.Report.get(62).outputParam}";
                        reportlink = reportlink + "&reportUnit=${icbs.admin.Report.get(62).reportUnit}";
                        reportlink = reportlink + "&transaction_id=${txnFileInstance.id}";
                        reportlink = reportlink + "&generatedby=${icbs.admin.UserMaster.get(session.user_id).username}";
                        sendtojasperver(reportlink);
                    } 

        function genReportVSLP003(){
                         reportlink = "${icbs.admin.Institution.findByParamCode("GEN.10251").paramValue}${icbs.admin.Report.get(64).baseParams}&output=${icbs.admin.Report.get(64).outputParam}";
                        reportlink = reportlink + "&reportUnit=${icbs.admin.Report.get(64).reportUnit}";
                        reportlink = reportlink + "&txn_file_id=${txnFileInstance.id}";
                        reportlink = reportlink + "&generatedby=${icbs.admin.UserMaster.get(session.user_id).username}";
                        sendtojasperver(reportlink);
                    }
            ...
            <g:if test="${print=='PrintValidation-PrintTransactionSlip-PrintPassbook'}">
                <div id="SlipTransaction" class="alert alert-success alert-dismissable" role="alert">
                   <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                        <div>
                            <%-- <a onclick="javascript:genReportVSLP003()"> --%>
