...
<div>
    ...
    <div name="add-deduction-form">
        <div class="fieldcontain form-group ${hasErrors(bean: deduction, field: 'scheme', 'has-error')}">
            ...
            <div class="col-sm-8">
                <g:select class="form-control" id="deductionScheme" name="scheme.id" from="${product?.loanDeductionSchemes?.findAll{it.status.id == 2}.sort{it.name}}" optionKey="id" optionValue="name" value="${deduction?.scheme?.id}" class="many-to-one form-control" onchange="updateDeductionForm()" />
