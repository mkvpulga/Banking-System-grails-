...
action:'getSchemeDetailsAjax')}", 
    ...
    <content tag="breadcrumbs">
        <span class="fa fa-chevron-right"></span><span class="current">Create Loan Account</span>
    </content>
    ...
    <content tag="main-actions">
        <ul>
           ...
           -->
            <li><button onclick="validateXistingLoanApplication();">Save</button></li>
            <li><g:link class="list" action="index" onclick="return confirm('${message(code: 'default.button.delete.confirm.message', default: 'Are you sure?')}');">Cancel</g:link></li>
                <script>
                function validateXistingLoanApplication(){
                var loanApplicationID = $('#forvalidateLoanApplicationId').val();

                console.log("execute");

                console.log("loanApplicationID: "+loanApplicationID);
                if(loanApplicationID==""){
                console.log("brad walang laman yung loan applicationm");
                }else{
                console.log("brad meron kaya i verify natejn");
                var obj = { 
                ...
                contentType: "application/json",
                                    url: "${createLink(controller:'Loan', action:'validateExistingLoanApplicationId')}",
                data: JSON.stringify(obj),

                success: function(data){
                                        if(data.length > 0){
                console.log("meorn syang nakita");
                alertify.alert(AppTitle,"This Loan Application is being used by other Loan Account!", function(){
                alertify.error('Please Change your Loan Application..');

                });

                }
                else{
                console.log("wala syang nakita")
