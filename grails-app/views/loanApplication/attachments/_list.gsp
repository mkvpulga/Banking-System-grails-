...
<div class="table-responsive">
	...
	<tbody>
		<g:if test="${loanApplicationInstance?.attachments}">
			<g:each var="attachment" in="${loanApplicationInstance?.attachments.sort{it.fileName}}">
				<tr>
		...
		<g:elseif test="${session["loanApplicationAttchmt"]}">
			<g:set var="i" value="${0}" />
			<g:each var="attachment" in="${session["loanApplicationAttchmt"]}">
				<tr>
					<td>${attachment?.fileName}</td>
					<td>${attachment?.description}</td>
					<td>${attachment?.type?.description}</td>
