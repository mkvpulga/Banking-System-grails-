...
class DepositPeriodicOpsService {
            ...
            for (ds in dl) {
                ...
                if (d.status.id == 5 && chargeOnlyBelowAdb == 'TRUE') {
                    ...
                    def ledger = new TxnDepositAcctLedger(acct:d, acctNo:d.acctNo, bal:d.ledgerBalAmt - chgAmt,
                        ...
                        passbookBal:0.00D,txnType:txn.txnType, txnCode:txn.txnCode)
                    ledger.save(flush:true, failOnError:true)
    ...
    private def timeDepositDailyInterestCalculation(Date currentDate, Branch branch){
        ...
        for(f in fdList){
            ...
            // === recompute tax jm marquez
            if ((fd.maturityDate - fd.dateOpened) > 1825) {
                //if holding period up to 1 year
                if ((fd.branch.runDate - fd.dateOpened) <= 365 ) {
                    preTermIntAmt = fd.ledgerBalAmt * duration_days * (fd.interestRate * 0.50D).div(fd.depositInterestScheme.divisor).div(100)
                }
                //if holding period greater than 1 year up to 2 years
                else if ((fd.branch.runDate - fd.dateOpened) > 365 && (fd.branch.runDate - fd.dateOpened) < 731 ) {
                    preTermIntAmt = fd.ledgerBalAmt * duration_days * (fd.interestRate * 0.60D).div(fd.depositInterestScheme.divisor).div(100)
                }
                //if holding period greater than 2 years up to 3 years
                else if ((fd.branch.runDate - fd.dateOpened) > 730 && (fd.branch.runDate - fd.dateOpened) < 1096 ) {
                    preTermIntAmt = fd.ledgerBalAmt * duration_days * (fd.interestRate * 0.70D).div(fd.depositInterestScheme.divisor).div(100)
                }
                //if holding period greater than 3 years up to 4 years
                else if ((fd.branch.runDate - fd.dateOpened) > 1095 && (fd.branch.runDate - fd.dateOpened) < 1461 ) {
                    preTermIntAmt = fd.ledgerBalAmt * duration_days * (fd.interestRate * 0.80D).div(fd.depositInterestScheme.divisor).div(100)
                }
                //if holding period greater than 4 years but less than 5 years
                else{
                    preTermIntAmt = fd.ledgerBalAmt * duration_days * (fd.interestRate * 0.90D).div(fd.depositInterestScheme.divisor).div(100)
                }
                //Computation of interest in case of pre-termination and proceeds of time deposit.
                // if less than 3 years interest shall be subject to withholding tax 20%
                if((fd.branch.runDate - fd.dateOpened) < 1095 ){
                    preTermTax = preTermIntAmt * 0.20D
                }
                // if 3 years but less than 4 years interest shall be subject to withholding tax 12%
                else if((fd.branch.runDate - fd.dateOpened) > 1094 && (fd.branch.runDate - fd.dateOpened) < 1460 ){
                    preTermTax = preTermIntAmt * 0.12D
                }
                // if 4 years but less than 5 years interest shall be subject to withholding tax 5%
                else{
                    preTermTax = preTermIntAmt * 0.05D
                }
                preTermIntAmt = preTermIntAmt.round(2)
                preTermTax = preTermTax.round(2)
            }            
        ...
        for (dx in depositList){
            ...
            if ((deposit.acrintAmt > 0) && (deposit.depositInterestScheme.depositCapitalizationFreq.id == 2) && (quarters.indexOf( refMonth ) >= 0)){
                ...
                if (Institution.findByParamCode('DEP.40130').paramValue == 'TRUE') {
                    if (deposit.accruedIntForTheMonth > 0.00D){
                        GlTransactionService.saveQuarterlyAccrual(tfInt.id)
                    }                  
                }
            ...
            println("deposit.accruedTaxForTheMonth: "+deposit.accruedTaxForTheMonth)
            println("deposit.depositInterestScheme.depositCapitalizationFreq.id: "+deposit.depositInterestScheme.depositCapitalizationFreq.id)
            println("refmonth: "+refMonth)
            def zxczxSSS = quarters.indexOf(refMonth)
            println("zxczxSSS index OF: "+zxczxSSS)
    ...
    private def depositInterestCalculation(Date currentDate,Branch branch){
        ...
        for(dx in depositList){
            deposit = Deposit.get(dx.id)
            println("===========================depositInterestCalculation")
            println 'EOM Check ---------------------------------------------'
            println dx.id
            ...
            //println("dx.avgBal / days"+ dx.avgBal days)
            ...
            println("deposit.depositInterestScheme.depositInterestCalculation.id: "+deposit.depositInterestScheme.depositInterestCalculation.id)
            if(deposit.depositInterestScheme.depositInterestCalculation.id ==1 ){
                println("interest Calculation 1")
                interestRate = deposit.interestRate.div(100)
                divisor = deposit.depositInterestScheme.divisor
                println("averageBal: "+averageBal)
                println("divisor: "+divisor)
                println("days = dx.numDays: "+dx.numDays)
                println("interestRate: "+interestRate)
                println("deposit.depositInterestScheme.minBalanceToEarnInterest: "+deposit.depositInterestScheme.minBalanceToEarnInterest)
                ...
                println("interestBalAmt: "+interestBalAmt)
                //deposit.ledgerBalAmt = deposit.ledgerBalAmt +interestBalAmt
                //deposit.availableBalAmt = deposit.availableBalAmt +interestBalAmt
                println("deposit.product.hasDepositDormancyInterest: "+deposit.product.hasDepositDormancyInterest)
                if(!deposit.product.hasDepositDormancyInterest){
                    println("true")
                }
                ...
                // check for AIP rule
                if (Institution.findByParamCode('DEP.40130').paramValue == 'TRUE') {
                    println("interestBalAmt.round(2): "+interestBalAmt.round(2))
                    deposit.accruedIntForTheMonth = interestBalAmt.round(2)
                    println("accruedIntForTheMonth: "+deposit.accruedIntForTheMonth)
                    ...
                    println("IC : 1 deposit.accruedIntForTheMonth: "+deposit.accruedIntForTheMonth)
            ...
            if(deposit.depositInterestScheme.depositInterestCalculation.id ==2){
                 ...
                 */
                println("deposit.interestRate.div(100): "+deposit.interestRate.div(100))
                interestRate = deposit.interestRate.div(100)
                divisor = deposit.depositInterestScheme.divisor
                ...
                // check for AIP rule
                if (Institution.findByParamCode('DEP.40130').paramValue == 'TRUE') {
                    println("pumasok na dito")
                    deposit.accruedIntForTheMonth = interestBalAmt.round(2)
                    println("deposit.accruedIntForTheMonth: "+deposit.accruedIntForTheMonth)
                    deposit.accruedIntPayable += interestBalAmt.round(2)
                    deposit.accruedTaxForTheMonth =  interestBalAmt.round(2) * deposit.depositTaxChargeScheme.taxRate.div(100)
                    deposit.accruedTaxForTheMonth = deposit.accruedTaxForTheMonth.round(2)
                    println("new deposit.accruedIntForTheMonth: "+deposit.accruedIntForTheMonth)
                    deposit.accruedTaxPayable += (interestBalAmt.round(2) * deposit.depositTaxChargeScheme.taxRate.div(100))
                    deposit.accruedTaxPayable = deposit.accruedTaxPayable.round(2)
                }
                deposit.save(flush:true)
        ...
        if (deposit.depositInterestScheme.interestOnClosing && (deposit.type.id == 1 || deposit.type.id == 2) ){
            ...
            if(deposit.depositInterestScheme.depositInterestCalculation.id == 1){
                ...
                if (days != 0) {
                    ...
                    averageBal = deposit.availableBal + deposit.holdBal
                    def divisor = deposit.depositInterestScheme.divisor
                    interestBalAmt = (averageBal*days).div(divisor)*interestRate
                    interestBalAmt = interestBalAmt.round(2)
                }
