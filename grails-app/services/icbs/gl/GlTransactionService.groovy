...
class GlTransactionService {
	...
	def glBatchHdr = GlBatchHdr.findByBatchId(glUpdateList.batchId)
        ...
        println("depstatus: "+depstatus)
        println("depositAcct.glLink: "+depositAcct.glLink)
        println("gla: "+gla)
        println("depositAcct.branch: "+depositAcct.branch)
        println("gla.glCode: "+gla.glCode)
        println("ccy "+ccy)
    ...
    def saveBatchEntry (params,theNewBuildLineNumber) {
        ...
        //check first for duplicates before saving
        def glb =  GlBatch.findByBatchIdAndLineNo(params.batchId,params.lineNo)
        if (glb) {
            auditLogService.insert('140', 'GEN00500', 'Error in Create GL Batch '+params.batchId+' Line '+params.lineNo, 'glBatch', null, null, null, null)
        } else {
    ...
    def saveBatchHeader (params) {
        ...
        if (b) {
            ...
            println("valueDAte : "+params.valueDate)
    ...
    def saveGlAccountsToMultipleBranches (params){
        ...
        // insert to gl_daily_file of previous day for Trial balance
    ...
    def PeriodicGlEntries(Date currentDate, UserMaster user) {
        ...
        for (b in bl) {
            ...
            def ccy = Currency.list()
            for(bc in ccy) {
                sqlstmt  = "select debit_acct as debitAcct, credit_acct as creditAcct, sum(debit_amt) as debitAmt, sum(credit_amt) as creditAmt " +
    ...
    def cociDeposit(TxnFile tf, TxnTemplate tmp, Integer fy) {
        def tbDr = new TxnBreakdown(currency:Currency.get(tf.currency.id), txnDate:tf.txnDate, txnFile:tf, user:UserMaster.get(tf.user.id), branch:tf.branch)
        def tbCr = new TxnBreakdown(currency:Currency.get(tf.currency.id), txnDate:tf.txnDate, txnFile:tf, user:UserMaster.get(tf.user.id), branch:tf.branch)
           
        def userCoci = UserMaster.get(tf.user.id)
        println("=======================gieeeeeeeee========>")
        println("userCoci : "+ userCoci)
        def debitGl =  GlAccount.findByCodeAndBranchAndFinancialYear(tmp.defContraAcct,tf.branch, fy)
        println("debitGl : "+ debitGl)             
        tbDr.debitAcct = debitGl.code
        println("tbDr.debitAcct : "+ tbDr.debitAcct) 
        tbDr.debitAmt = tf.txnAmt
        println("tbDr.debitAmt : "+ tbDr.debitAmt) 
        println("userCoci.coci.id : "+ userCoci.coci.id)
        println("**************************")
        def creditGl = GlAccount.get(userCoci.coci.id)
        println("creditGl : "+ creditGl)
        tbCr.creditAcct = creditGl.code
        println("tbCr.creditAcct : "+ tbCr.creditAcct)
        tbCr.creditAmt = tf.txnAmt
        println("tbCr.creditAmt : "+ tbCr.creditAmt)
        if (!creditGl){
            tbCr.creditAcct = GlAccount.findByCodeAndBranchAndFinancialYear(Institution.findByParamCode('GLS.60070').paramValue,tf.branch, fy)
        }
        if (!debitGl){
        ...
        if (atmTxnInstance.atmTerminal) {
            ...
            if (atmTerminal) {
                if (atmTerminal.branch == tf.branch) {
                    ...
                    tbCrIbt.save(flush:true, failOnError: true)
    ...
    def saveLoanMemoPayment(TxnFile tf, TxnTemplate tmp, Integer fy){
        def tbDr = new TxnBreakdown(currency:Currency.get(tf.currency.id), txnDate:tf.txnDate, txnFile:tf, user:UserMaster.get(tf.user.id), branch:tf.branch)
        def tbCr = new TxnBreakdown(currency:Currency.get(tf.currency.id), txnDate:tf.txnDate, txnFile:tf, user:UserMaster.get(tf.user.id), branch:tf.branch)
        ...
        if (tmp.memoTxnType.id == 4) {
            ...
            if (lnPay.otherAmt > 0) {
                def tbCrPen = new TxnBreakdown(currency:Currency.get(tf.currency.id), txnDate:tf.txnDate, txnFile:tf, user:UserMaster.get(tf.user.id), branch:loan.branch)
                def crGlPenPtr = CfgAcctGlTemplateDet.findByGlTemplateAndOrdinalPosAndStatus(loan.glLink, 'a', loan.loanPerformanceId.id)
                def crGlPen = GlAccount.findByCodeAndBranchAndFinancialYear(crGlPenPtr.glCode,loan.branch, fy)
                tbCrPen.creditAcct = crGlPen.code
                tbCrPen.creditAmt = lnPay.otherAmt
                tbCrPen.save(flush:true)
            }
            ...
            def lnPayOthers = TxnLoanPaymentOthers.findByTxnLoanPaymentDetails(lnPay)
            if (lnPayOthers){
                if (lnPayOthers.arOthers > 0.00D) {
                    def tbCrAr = new TxnBreakdown(currency:Currency.get(tf.currency.id), txnDate:tf.txnDate, txnFile:tf, user:UserMaster.get(tf.user.id), branch:loan.branch)
                    tbCrAr.creditAcct = lnPayOthers.arOthersGl
                    tbCrAr.creditAmt = lnPayOthers.arOthers
                    tbCrAr.save(flush:true)
                }
                if (lnPayOthers.insurance > 0.00D) {
                    def tbCrIns = new TxnBreakdown(currency:Currency.get(tf.currency.id), txnDate:tf.txnDate, txnFile:tf, user:UserMaster.get(tf.user.id), branch:loan.branch)
                    tbCrIns.creditAcct = lnPayOthers.insuranceGl
                    tbCrIns.creditAmt = lnPayOthers.insurance
                    tbCrIns.save(flush:true)
                }
                if (lnPayOthers.depositCr > 0.00) {
                    def tmpCr = TxnTemplate.get(Institution.findByParamCode('DEP.40122').paramValue.toInteger())
                    def tbCrDep = new TxnBreakdown(currency:Currency.get(tf.currency.id), txnDate:tf.txnDate, txnFile:tf, user:UserMaster.get(tf.user.id), branch:loan.branch)
                    tbCrDep.creditAcct = tmpCr.defContraAcct
                    tbCrDep.creditAmt = lnPayOthers.depositCr
                    tbCrDep.save(flush:true)
                }
            }
        }
    ...
    def saveTxnBreakdown (Long txnId){
        ...
        if (tmp.txnType.id == 67) {
            def posted = cociDeposit(tf, tmp, fy)
        }
        ...
        if (tmp.txnType.id == 12 || tmp.txnType.id == 13 || tmp.txnType.id == 14 || tmp.txnType.id == 15){
            ...
            if (lnPay.penaltyAmt > 0) {
                ...
                def crGlIPtr = CfgAcctGlTemplateDet.findByGlTemplateAndOrdinalPosAndStatus(loan.glLink, '4', loan.loanPerformanceId.id)
                def crGlI = GlAccount.findByCodeAndBranchAndFinancialYear(crGlIPtr.glCode,loan.branch, fy)
                tbCrInt.creditAcct = crGlI.code
                tbCrInt.creditAmt = lnPay.penaltyAmt
                tbCrInt.save(flush:true)
            }
            if (lnPay.otherAmt > 0) {
                def tbCrPen = new TxnBreakdown(currency:Currency.get(tf.currency.id), txnDate:tf.txnDate, txnFile:tf, user:UserMaster.get(tf.user.id), branch:loan.branch)
        ...
        if ((tmp.txnType.id == 16 || tmp.txnType.id == 38) && (tmp.memoTxnType.id == 4 || tmp.memoTxnType.id == 7 || tmp.memoTxnType.id == 10)){
            def posted = saveLoanMemoPayment(tf, tmp, fy)
        }
        // loan write-off
    ...
    def saveQuarterlyAccrual(Long txnId){
        ...
        tbCr.creditAcct = creditGl.code
        tbCr.creditAmt = deposit.accruedIntForTheMonth
        if (!creditGl){
            tbCr.creditAcct = GlAccount.findByCodeAndBranchAndFinancialYear(Institution.findByParamCode('GLS.60070').paramValue,tf.branch, fy)
        }
        tbCr.save()      
        ...
        //            def tbDrTax = new TxnBreakdown(currency:Currency.get(tfTax.currency.id), txnDate:tfTax.txnDate, txnFile:tf, user:UserMaster.get(tfTax.user.id), 
        //                branch:tfTax.branch)
        //            def tbCrTax = new TxnBreakdown(currency:Currency.get(tfTax.currency.id), txnDate:tfTax.txnDate, txnFile:tf, user:UserMaster.get(tfTax.user.id), 
        //                branch:tfTax.branch)           
        //
        //            def debitGlPtrTax = CfgAcctGlTemplateDet.findByGlTemplateAndOrdinalPosAndStatus(deposit.glLink, '1', depStatus)
        //            def debitGlTax = GlAccount.findByCodeAndBranchAndFinancialYear(debitGlPtrTax.glCode,tf.branch, fy)                
        //            tbDrTax.debitAcct = debitGlTax.code
        //            tbDrTax.debitAmt = deposit.accruedTaxForTheMonth
        //            if (!debitGl){
        //                tbDr.debitAcct = GlAccount.findByCodeAndBranchAndFinancialYear(Institution.findByParamCode('GLS.60070').paramValue,tf.branch, fy)
        //            }
        //
        ...
        //            def creditGlPtrTax = CfgAcctGlTemplateDet.findByGlTemplateAndOrdinalPosAndStatus(deposit.glLink, '3', depStatus)
        //            def creditGlTax = GlAccount.findByCodeAndBranchAndFinancialYear(creditGlPtrTax.glCode,tf.branch, fy)
        //            tbCrTax.creditAmt = deposit.accruedTaxForTheMonth
        //            tbCrTax.creditAcct = creditGlTax.code           
        //            tbCrTax.save()               
        //        }        
    }
